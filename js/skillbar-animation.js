...
for (var i = 0; i < doughnutArray.length; i++) {
  doughnutArray[i].lineWidth = 15; //thickness of the line
  doughnutArray[i].fillStyle = '#fff';
  doughnutArray[i].strokeStyle = "#fff";
  doughnutArray[i].beginPath();
  doughnutArray[i].arc(70, 70, 55, 4.72, 15, false); //.arc(x, y , radius, startAngle, endAngle, anticlockwise)
...
function loadSkills1() {
  ...
  function progressSim() {
    ...
    ctx.fillStyle = '#e73d28';
    ctx.strokeStyle = "#e73d28";
    ctx.textAlign = 'center';
    ctx.font = "23px Radley";
    ctx.fillText(al + '%', cw * .5 + 2, ch * .5 + 8, cw);
    ctx.beginPath();
    ctx.arc(70, 70, 55, start, diff / 10 + start, false); //.arc(x, y , radius, startAngle, endAngle, anticlockwise)
    ctx.stroke();
